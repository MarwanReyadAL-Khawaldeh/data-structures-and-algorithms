...
const getNames = (arr) => {
  ...
  const arr2 = [];
  arr.map(value => {
    arr2.push(value.name.split('').reverse().join(''));
  });
  return arr2;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.reduce((total, val) => {
    total.push(val.reduce((total2, val2) => {
      if (val2 % 5 === 0 && typeof val2 === 'number') {
        total2.push(Math.pow(2, val2));
      }
      return total2;
    }, []));
    return total;
...
let findShortest = (data) => {
  // Solution code here...
  const shortName = data.reduce((total, val) => {
    if (parseInt(val.height) < total.height) {
      total = val;
    }
    return total;
  });
